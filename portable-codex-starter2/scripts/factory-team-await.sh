...
TIMEOUT_MS="${FACTORY_TEAM_AWAIT_TIMEOUT_MS:-0}"
AFTER_EVENT_ID="${FACTORY_TEAM_AWAIT_AFTER_EVENT_ID:-}"
JSON_MODE=0
...
if ! command -v omx >/dev/null 2>&1; then
  echo "[ERROR] omx is required for factory-team-await." >&2
  exit 1
fi

args=(omx team await "${TEAM_NAME}")
if [[ "${TIMEOUT_MS}" =~ ^[0-9]+$ && "${TIMEOUT_MS}" != "0" ]]; then
  args+=(--timeout-ms "${TIMEOUT_MS}")
fi
if [[ -n "${AFTER_EVENT_ID}" ]]; then
  args+=(--after-event-id "${AFTER_EVENT_ID}")
fi
if [[ "${JSON_MODE}" == "1" ]]; then
  args+=(--json)
fi

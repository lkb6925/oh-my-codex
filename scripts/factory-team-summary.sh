...
if ! command -v omx >/dev/null 2>&1; then
  echo "[ERROR] omx is required for factory-team-summary." >&2
  exit 1
fi

if [[ "${JSON_MODE}" == "1" ]]; then
  omx team api get-summary --input "{\"team_name\":\"${TEAM_NAME}\"}" --json
else
  echo "# Factory Team Summary"
  echo ""
  echo "- Team: ${TEAM_NAME}"
  echo "- Status:"
  omx team status "${TEAM_NAME}" --tail-lines "${FACTORY_TEAM_TAIL_LINES:-40}"
  echo ""
  echo "- Summary:"
  omx team api get-summary --input "{\"team_name\":\"${TEAM_NAME}\"}" --json
fi
